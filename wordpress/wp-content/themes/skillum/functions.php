<?php
/**
 * Skillum functions and definitions.
 */
add_filter( 'kava-theme/assets-depends/styles', 'skillum_styles_depends' );
add_action( 'jet-theme-core/register-config', 'skillum_core_config' );
add_action( 'init', 'skillum_plugins_wizard_config', 9 );
add_action( 'init', 'skillum_data_importer_config', 9 );
add_action( 'tgmpa_register', 'skillum_register_required_plugins' );
/**
 * Enqueue styles.
 */
function skillum_styles_depends( $deps ) {
 ...
 */
function skillum_core_config( $manager ) {
	$manager->register_config(
		array(
			'dashboard_page_name' => esc_html__( 'Skillum', 'skillum' ),
 ...
 * @return [type] [description]
 */
function skillum_plugins_wizard_config() {
	if ( ! is_admin() ) {
		return;
	}
		...
		'skins'   => array(
			...
			'advanced' => array(
				'default' => array(
					...
					'name'  => esc_html__( 'Skillum', 'skillum' ),
					),
				),
			),
		'texts'   => array(
			'theme-name' => esc_html__( 'Skillum', 'skillum' ),
		)
	) );
}
/**
 ...
 * @return [type] [description]
 */
function skillum_data_importer_config() {
	if ( ! is_admin() ) {
		return;
	}
	...
	jet_data_importer_register_config( array(
		...
		'advanced_import' => array(
			'default' => array(
				'label'    => esc_html__( 'Skillum', 'skillum' ),
				...
				'demo_url' => 'https://ld-wp73.template-help.com/wordpress/prod_20708/v2/',
			),
		),
		'import' => array(
			'chunk_size' => 3,
...
*/
		...
		'slider' => array(
			'path' => 'https://raw.githubusercontent.com/ZemezLab/kava-skillum/master/slides.json',
		),
	) );
}
/**
 ...
 */
function skillum_register_required_plugins() {
	$plugins = array(
		array(
			'name'         => esc_html__( 'Jet Plugin Wizard', 'skillum' ),
